...
const AddUser = ({ }) => {
  ...
  const [userRegisterFields, setUserRegisterFields] = useState({ "firstName": "", "lastName": "", "dateOfBirth": null, "password": "", "mobileNumber": "", "emailId": "", "userRole": "", "confirmPassword": "","registeredClientName":"" })
  ...
  const onUserRegister = async (e) => {
    ...
    if (userRegisterFields.password === userRegisterFields.confirmPassword) {
      ...
      if (payload?.statusCode === 200) {
        setUserRegisterFields({ "firstName": "", "lastName": "", "dateOfBirth": null, "password": "", "mobileNumber": "", "emailId": "", "userRole": "", "confirmPassword": "" ,"registeredClientName":""})
  ...
  return (
    <>
      ...
      <DynamicHead title={` USER'S REPORT OF ${fullName?.toLocaleUpperCase()}`} />
...
{
  userRegisterFields?.userRole===FinoLabel.finoUserRoles[3] && <Grid container>

  <Grid item xs={12} md={3}>
    <Box sx={{ p: 1, ...GlobalStyles.alignmentStyles }}>

      <CustomTextField 
     value={userRegisterFields?.registeredClientName}
     onChange={(e) => { setUserRegisterFields({ ...userRegisterFields, registeredClientName: e.target.value }) }}
       isFullwidth={true} label={"Client Name"} 
